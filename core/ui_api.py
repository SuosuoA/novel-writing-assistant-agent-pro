...
class GenerationServiceProxy:
    ...
    def generate_chapter(
        self, title: str, outline: str, word_count: int = 2000, max_iterations: int = 5
    ) -> Dict[str, Any]:
        """
        生成章节
        ...
        if self._event_bus:
            self._event_bus.publish(
                "generation.requested", {"request": request.model_dump()}
            )
    ...
    def cancel_generation(self, request_id: str) -> bool:
        ...
        """
        if self._event_bus:
            self._event_bus.publish("generation.cancelled", {"request_id": request_id})
        return True
    ...
    def _on_progress(self, event: Any) -> None:
        ...
        def update():
            self._progress_callback(
                event.data.get("progress", 0), event.data.get("message", "")
            )
...
class EventServiceProxy:
    ...
    def subscribe(
        ...
        return self._event_bus.subscribe(event_type, safe_handler, handler_id)
