...
class SecureConfig:
    ...
    def __init__(
        ...
        """
        self._config_path = config_path or Path.home() / ".novel-assistant" / "secure_config.json"
    ...
    def _init_encryption(self, encryption_key: Optional[bytes] = None) -> None:
        ...
        if not ENCRYPTION_AVAILABLE:
            logger.warning("Cryptography library not available, encrypted storage disabled")
            return
        ...
        try:
            if encryption_key:
                ...
                if key_path.exists():
                    ...
                    # 生成新密钥（基于机器信息）
                    import hashlib
                    machine_id = os.uname().nodename if hasattr(os, 'uname') else os.getenv('COMPUTERNAME', 'default')
    ...
    def _save_config(self) -> None:
        ...
        try:
            ...
            safe_config = {
                ...
                if not any(s in k.lower() for s in ["api_key", "secret", "password", "token"])
            }
