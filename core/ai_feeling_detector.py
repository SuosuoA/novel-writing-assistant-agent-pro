...
def build_anti_ai_prompt_guidance(max_words: int = 28) -> str:
    """构建"降低AI感、追求以假乱真"的生成指导文本（单一真值来源）。

    与检测器共用同一份AI痕迹清单（AI_COMMON_WORDS）——检测器事后识别AI腔，
    此指导前置注入各生成路径（普通/专家/续写/降级），让模型从源头规避。
    这样"检测→反馈→规避"形成闭环，且全路径一致。

    Args:
        max_words: 示例中列出的AI高频词数量上限（控制prompt长度）

    Returns:
        可直接拼入生成prompt的指导段落
    """
    try:
        ai_words = list(AIFeelingDetector.AI_COMMON_WORDS)
    except Exception:
        ai_words = ["仿佛", "似乎", "不禁", "不由得", "渐渐", "心中",
                    "一股", "莫名", "终于", "一切都", "然而", "彷佛"]
    sample = "、".join(ai_words[:max_words])

    lines = [
        "=" * 60,
        "文风要求（降低AI感，追求以假乱真，极其重要）",
        "=" * 60,
        "目标：让读者以为是资深人类作者写的，而非AI生成。务必做到：",
        "",
        "1. 克制AI高频词（勿滥用，优先用具体表达替代）：",
        f"   {sample} 等。",
        "   反例：「他心中涌起一股莫名的情绪」→ 写出具体动作/念头/生理反应",
        "   （攥紧了茶杯／喉头发紧／盯着桌角那道裂纹）。",
        "",
        "2. 拒绝模板化的开头与结尾：",
        "   - 开头别用「这是一个…的日子」「今天天气…」「随着…的到来」，",
        "     直接切入具体场景、动作或一句对话。",
        "   - 结尾别用「就这样，一切都结束了」「他终于明白了…」这类总结，",
        "     用一个具体画面、一个动作或一句没说完的话收束。",
        "",
        "3. 展示而非告知（show, don't tell）：",
        "   - 不写「她很愤怒」，写她做了什么、说了什么、身体如何反应；",
        "   - 不写「气氛很紧张」，用环境细节和小动作让读者自己感到紧张；",
        "   - 情绪藏在动作、对话、留白里，不要直接命名情绪。",
        "",
        "4. 避免过度解释与论文腔：",
        "   - 不做「因为…所以…导致…」式因果说明，让情节自己说话；",
        "   - 少用「不仅…而且…甚至…」「一方面…另一方面…」「首先…其次…」；",
        "   - 不替读者总结「这说明…」「由此可见…」。",
        "",
        "5. 句子节奏要像人：",
        "   - 长短句交错，该短促时就用三五字的短句，别每句都工整对仗；",
        "   - 允许残句、口语、停顿；对白带上人物语气和口头习惯；",
        "   - 同一段避免连续用「然后／接着／于是」推进。",
        "",
        "6. 细节的真实感：",
        "   - 用具体、有质感的名词动词（不是「一些东西」而是「半块凉透的烧饼」）；",
        "   - 比喻要新鲜、贴合此情此景，避免「像花一样美」这类陈词滥调；",
        "   - 该粗粝就粗粝，人物不必句句得体，世界不必处处圆满。",
        "",
    ]
    return "\n".join(lines)
