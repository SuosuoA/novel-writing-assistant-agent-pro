...
├── event_bus.py         # 事件总线（发布订阅机制）- V1.2完善异步实现
├── plugin_registry.py   # 插件注册表（状态管理）- V1.1已修正
├── service_locator.py   # 服务定位器（依赖注入+生命周期管理）- V1.1已增强
├── config_manager.py    # 配置管理器（含验证和写锁）
├── plugin_loader.py     # 插件加载器（动态导入+热插拔+依赖解析）- V1.1已增强
├── plugin_interface.py  # 插件接口定义
├── models.py            # Pydantic数据模型
├── ui_api.py            # UI层交互API规范
├── circuit_breaker.py   # 熔断器（V1.2新增）
├── secure_config.py     # 安全配置管理（V1.2新增）
├── log_sanitizer.py     # 日志脱敏（V1.2新增）
├── plugin_signer.py     # 插件签名器（V1.2新增）
└── OpenClaw mem9架构相关模块（V1.5新增）：
    ├── session_state.py         # L1热记忆：SESSION-STATE管理
    ├── wal_manager.py           # WAL协议：原子化写入
    ├── knowledge_manager.py     # 知识管理：CRUD操作
    ├── knowledge_retriever.py   # 知识检索：向量召回
    ├── knowledge_recall.py      # 知识召回：智能召回
    ├── chapter_encoder.py       # 章节编码：向量编码器
    ├── context_recall.py        # 上下文召回：智能召回器
    ├── conflict_fixer.py        # 冲突修复：修复建议
    └── git_notes_manager.py     # L3冷记忆：Git-Notes管理
```

**V1.2新增模块**：
- `circuit_breaker.py`：故障自动恢复（CLOSED→OPEN→HALF_OPEN→CLOSED）
- `secure_config.py`：API密钥安全存储（系统密钥环+加密配置）
- `log_sanitizer.py`：日志敏感信息脱敏
- `plugin_signer.py`：插件代码签名与验证

**V1.5新增模块（OpenClaw mem9架构）**：
- `session_state.py`：L1热记忆管理，实时会话状态
- `wal_manager.py`：WAL协议，原子化写入保证数据一致性
- `knowledge_manager.py`：知识库CRUD操作
- `knowledge_retriever.py`：向量检索引擎
- `knowledge_recall.py`：智能知识召回
- `chapter_encoder.py`：章节内容向量化
- `context_recall.py`：上下文智能召回
- `conflict_fixer.py`：一致性冲突修复
- `git_notes_manager.py`：Git-Notes历史决策管理
