...
class CustomTitleBar(tk.Frame):
    ...
    def _minimize_window(self) -> None:
        """最小化窗口，兼容 override-redirect 模式。
        
        override-redirect 窗口不支持 iconify()，使用
        overrideredirect(False) → iconify() → 还原时重设 override-redirect 方式。
        """
        try:
            # 先临时关闭 override-redirect，再最小化
            self.parent.overrideredirect(False)
            self.parent.iconify()
            # 当窗口从最小化恢复时，重新设置 override-redirect
            self.parent.bind("<Map>", self._on_deiconify)
        except Exception:
            # 降级：直接 withdraw（窗口消失，不进任务栏）
            self.parent.withdraw()

    def _on_deiconify(self, event=None) -> None:
        """窗口从最小化恢复时，重新启用 override-redirect 无边框"""
        try:
...
class MainWindow:
    ...
    def _create_generation_content(self) -> tk.Frame:
        ...
        # 分类显示名称映射（内部key -> 中文显示名）- 完整题材分类（14个）
    ...
    def _update_kb_options(self, stats: dict):
        ...
        try:
            ...
            if stats.get("source") == "json_cache":
                ...
                raw_categories = list(domains.keys())
            else:
                # 从向量库获取分类
            ...
            for category in self._kb_categories:
                ...
                count = 0
                if stats.get("source") == "json_cache":
                    domain_counts = stats.get("domains", {}).get(category, {})
                    count = sum(domain_counts.values()) if isinstance(domain_counts, dict) else 0
                else:
                    count = stats.get("categories", {}).get(category, 0)
    ...
    def _load_writing_techniques_from_knowledge_base(self) -> Dict[str, List[str]]:
        ...
        for domain_file in knowledge_dir.glob("*.json"):
            ...
            try:
                ...
                if titles:
                    ...
                    logger.warning(f"{domain_file.name} 中没有找到知识点")
        ...
        # 如果没有加载到任何技巧，使用默认值
        if not techniques:
            logger.warning("未从知识库加载到任何写作技巧，使用默认值")
            return self._get_default_writing_techniques()
    ...
    def _load_writing_technique_stats(self) -> Dict[str, Any]:
        ...
        # 遍历每个领域的JSON文件
        for domain_file in knowledge_dir.glob("*.json"):
            domain_key = domain_file.stem
            
            # 跳过索引文件
            if domain_key == "index":
                continue
            
            # 处理文件名前缀（writing_technique_前缀）
            if domain_key.startswith("writing_technique_"):
                domain_key = domain_key[len("writing_technique_"):]
...
"""
    ...
    def _create_knowledge_content(self) -> tk.Frame:
        ...
        self._domain_map = {
            ...
            # 历史相关
            "政治": "politics", "军事": "military", "经济": "economics",
            # 科幻相关
            ...
            # 游戏相关
            "游戏设计": "design", "叙事": "narrative",
            # 灵异相关
            "民俗": "folklore", "传说": "legend", "神秘学": "occultism",
            # 同人相关
            "原著分析": "analysis", "人物": "characters", "剧情": "plot", "设定": "setting",
            # 通用写作
            "写作技法": "writing_technique", "叙事结构": "narrative", "修辞": "rhetoric",
            "人物塑造": "character", "情节设计": "plot_design", "对话": "dialogue",
            # 其他
            "逻辑": "logic", "基础知识": "basic_knowledge",
            # V5.3新增：写作技巧六领域（按12.2文档规范）
            "叙事技巧": "narrative",
            "描写技巧": "description",
            "修辞技巧": "rhetoric",
            "结构技巧": "structure",
    ...
    def _on_knowledge_stats(self):
        ...
        try:
            ...
            import json
            text.insert("1.0", json.dumps(stats, indent=2, ensure_ascii=False))
...
保护: {'是 (V5核心模块)' if plugin_data.get('is_protected') else '否'}
    ...
    def _on_edit_memory_md(self):
        ...
        if not memory_file.exists():
            messagebox.showwarning("提示", "MEMORY.md文件不存在")
            return
