...
class GlassWindowManager:
    ...
    def make_frameless(self) -> None:
        """设置完全无边框窗口（移除系统边框）"""
        if not self._dll_available or not self.hwnd:
            return
        
        try:
            # 移除标题栏和系统边框，只保留调整大小的边框
            style = self.user32.GetWindowLongW(self.hwnd, self.GWL_STYLE)
            # 移除标题栏、系统菜单，但保留调整大小和最小化/最大化功能
...
class CustomTitleBar(tk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        self.min_btn = tk.Label(
            btn_frame,
            text="—",
            font=(GlassTheme.FONT_FAMILY, 12),
        ...
        self.close_btn = tk.Label(
            btn_frame,
            text="×",
            font=(GlassTheme.FONT_FAMILY, 14),
            fg=GlassTheme.TEXT_SECONDARY,
            bg=GlassTheme.GLASS_BG,
            width=4,
            cursor="hand2"
...
class MainWindow:
    ...
    def _enable_glass_effect(self) -> None:
        ...
        try:
            ...
            if glass_mgr.enable_acrylic(0x80000000):
                ...
                logger.info("Acrylic glass effect enabled with frameless window")
            else:
                # 降级到透明效果
                self.root.attributes('-alpha', 0.95)
                logger.info("Fallback to alpha transparency")
    ...
    def _create_hot_ranking_page(self) -> tk.Frame:
        ...
        site_badge_info = [
            ('番茄', '红番茄', GlassTheme.ACCENT_RED),
            ('起点', '起点', GlassTheme.INFO),
            ('晋江', '晋江', GlassTheme.ACCENT_PURPLE),
        ]
        ...
        for site_info in sites_data:
            site_column = ttk.Frame(sites_frame, style="TFrame")
            ...
            for idx, book in enumerate(books, 1):
                ...
                # 排名徽章
                rank_bg = site_info['color'] if idx <= 3 else GlassTheme.GLASS_BORDER
                rank_label = tk.Label(
                    book_item,
                    text=str(idx),
    ...
    def _create_workbench_page(self) -> tk.Frame:
        ...
        functions = [
            ("🌍 世界观管理", "worldview", GlassTheme.ACCENT_PURPLE),
            ("👥 人物设定", "characters", GlassTheme.ACCENT_PINK),
            ("📄 大纲管理", "outline", GlassTheme.PRIMARY_LIGHT),
            ("✍️ 风格学习", "style", GlassTheme.ACCENT_GREEN),
        ...
        self._workbench_content_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=(0, 20))
    ...
    def _create_worldview_content(self) -> tk.Frame:
        ...
        columns = ("name", "elements", "status", "modified")
        ...
        self._worldview_tree.column("name", width=150)
        self._worldview_tree.column("elements", width=200)
        self._worldview_tree.column("status", width=80)
        self._worldview_tree.column("modified", width=120)
        ...
        ttk.Button(list_btn_frame, text="查看详情", command=self._on_worldview_view).pack(side=tk.LEFT, padx=5)
        ttk.Button(list_btn_frame, text="编辑", command=self._on_worldview_edit).pack(side=tk.LEFT, padx=5)
        ttk.Button(list_btn_frame, text="批量删除", command=self._on_worldview_delete).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(btn_frame, text="导入世界观", command=self._on_worldview_import).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="关联要素", command=self._on_worldview_link).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="清除", command=self._on_worldview_clear).pack(side=tk.LEFT, padx=5)
    ...
    def _create_characters_content(self) -> tk.Frame:
        ...
        # 列表（增加列）
        columns = ("avatar", "name", "role", "status", "emotion", "chapters")
        self._character_tree = ttk.Treeview(list_frame, columns=columns, show="headings", height=8)
        self._character_tree.heading("avatar", text="头像")
        ...
        self._character_tree.heading("emotion", text="情绪")
        self._character_tree.heading("chapters", text="出场章节")
        self._character_tree.column("avatar", width=50)
        self._character_tree.column("name", width=100)
        self._character_tree.column("role", width=100)
        self._character_tree.column("status", width=80)
        self._character_tree.column("emotion", width=80)
        ...
        list_btn_frame = ttk.Frame(list_frame, style="TFrame")
        list_btn_frame.pack(fill=tk.X, pady=5)
        ttk.Button(list_btn_frame, text="编辑人物", command=self._on_character_edit).pack(side=tk.LEFT, padx=5)
        ttk.Button(list_btn_frame, text="人物详情", command=self._on_character_detail).pack(side=tk.LEFT, padx=5)
        ttk.Button(list_btn_frame, text="关系图谱", command=self._on_character_relation).pack(side=tk.LEFT, padx=5)
        ttk.Button(list_btn_frame, text="删除人物", command=self._on_character_delete).pack(side=tk.LEFT, padx=5)
    ...
    def _create_outline_content(self) -> tk.Frame:
        ...
        ttk.Button(tree_btn_frame, text="添加卷", command=self._on_outline_add_volume).pack(side=tk.LEFT, padx=5)
        ttk.Button(tree_btn_frame, text="添加章节", command=self._on_outline_add_chapter).pack(side=tk.LEFT, padx=5)
        ttk.Button(tree_btn_frame, text="编辑章节", command=self._on_outline_edit_chapter).pack(side=tk.LEFT, padx=5)
        ttk.Button(tree_btn_frame, text="删除", command=self._on_outline_delete).pack(side=tk.LEFT, padx=5)
        ...
        btn_frame = ttk.Frame(frame, style="TFrame")
        btn_frame.pack(fill=tk.X, padx=5, pady=5)
        ttk.Button(btn_frame, text="解析大纲", command=self._on_outline_parse).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="清除大纲", command=self._on_outline_clear).pack(side=tk.LEFT, padx=5)
    ...
    def _create_style_content(self) -> tk.Frame:
        ...
        ttk.Button(lib_btn_frame, text="切换风格", command=self._on_style_switch).pack(side=tk.LEFT, padx=5)
        ttk.Button(lib_btn_frame, text="编辑权重", command=self._on_style_edit_weight).pack(side=tk.LEFT, padx=5)
        ttk.Button(lib_btn_frame, text="删除风格", command=self._on_style_delete_profile).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(create_frame, text="深度学习", command=self._on_style_learn).pack(side=tk.LEFT, padx=5)
        ttk.Button(create_frame, text="创建风格模板", command=self._on_style_create).pack(side=tk.LEFT, padx=5)
        ttk.Button(create_frame, text="应用到生成器", command=self._on_style_apply).pack(side=tk.LEFT, padx=5)
        ttk.Button(create_frame, text="清除风格", command=self._on_style_clear).pack(side=tk.LEFT, padx=5)
    ...
    def _create_generation_content(self) -> tk.Frame:
        ...
        # 中部：生成过程展示
        process_frame = ttk.LabelFrame(frame, text="生成过程（评分反馈、循环优化）", padding=10)
        ...
        # 日志输出
        self._gen_log = tk.Text(process_frame, wrap=tk.WORD, height=8,
                               font=(GlassTheme.FONT_FAMILY_CODE, GlassTheme.FONT_SIZE_SMALL),
                               bg=GlassTheme.GLASS_SURFACE, fg=GlassTheme.TEXT_PRIMARY)
        self._gen_log.pack(fill=tk.BOTH, expand=True)
        
        # 下部：生成结果展示
        result_frame = ttk.LabelFrame(frame, text="生成结果预览", padding=10)
        result_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        self._gen_result = tk.Text(result_frame, wrap=tk.WORD, height=8,
                                  font=(GlassTheme.FONT_FAMILY, GlassTheme.FONT_SIZE_NORMAL),
                                  bg=GlassTheme.GLASS_SURFACE, fg=GlassTheme.TEXT_PRIMARY)
        ...
        btn_frame = ttk.Frame(frame, style="TFrame")
        btn_frame.pack(fill=tk.X, padx=5, pady=5)
        ttk.Button(btn_frame, text="开始生成", command=self._on_start_generation).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="停止生成", command=self._on_stop_generation).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="分章浏览", command=self._on_gen_browse).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="保存项目", command=self._on_gen_save).pack(side=tk.LEFT, padx=5)
    ...
    def _create_reverse_content(self) -> tk.Frame:
        ...
        ttk.Button(chapters_btn_frame, text="上传章节", command=self._on_upload_chapter).pack(side=tk.LEFT, padx=5)
        ttk.Button(chapters_btn_frame, text="删除选中", command=self._on_delete_completed_chapter).pack(side=tk.LEFT, padx=5)
        ttk.Button(chapters_btn_frame, text="全部标记完成", command=self._on_mark_all_completed).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(analysis_frame, text="运行分析", command=self._on_reverse_run).pack(pady=10)
        ...
        btn_frame = ttk.Frame(frame, style="TFrame")
        btn_frame.pack(fill=tk.X, padx=5, pady=5)
        ttk.Button(btn_frame, text="自动修正", command=self._on_reverse_apply).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="调整大纲细节", command=self._on_reverse_outline).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="修改人物性格", command=self._on_reverse_character).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="补充人物轨迹", command=self._on_reverse_trajectory).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="增减世界观设定", command=self._on_reverse_worldview).pack(side=tk.LEFT, padx=5)
    ...
    def _create_quick_content(self) -> tk.Frame:
        ...
        # 下部：四个生成结果框（2x2布局）
        results_outer_frame = ttk.LabelFrame(frame, text="一键生成结果（世界观、大纲、人设、关键情节）", padding=10)
        results_outer_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # 上排
        top_row = ttk.Frame(results_outer_frame, style="TFrame")
        top_row.pack(fill=tk.BOTH, expand=True, pady=(0, 5))
        
        # 世界观结果框
        worldview_frame = ttk.LabelFrame(top_row, text="🌍 世界观", padding=5)
        worldview_frame.pack(fill=tk.BOTH, expand=True, side=tk.LEFT, padx=(0, 5))
        self._quick_worldview_result = tk.Text(worldview_frame, wrap=tk.WORD, height=6,
                                               font=(GlassTheme.FONT_FAMILY, GlassTheme.FONT_SIZE_SMALL),
                                               bg=GlassTheme.GLASS_SURFACE, fg=GlassTheme.TEXT_PRIMARY)
        ...
        # 大纲结果框
        outline_frame = ttk.LabelFrame(top_row, text="📄 大纲", padding=5)
        outline_frame.pack(fill=tk.BOTH, expand=True, side=tk.RIGHT, padx=(5, 0))
        self._quick_outline_result = tk.Text(outline_frame, wrap=tk.WORD, height=6,
                                             font=(GlassTheme.FONT_FAMILY, GlassTheme.FONT_SIZE_SMALL),
                                             bg=GlassTheme.GLASS_SURFACE, fg=GlassTheme.TEXT_PRIMARY)
        ...
        # 下排
        bottom_row = ttk.Frame(results_outer_frame, style="TFrame")
        bottom_row.pack(fill=tk.BOTH, expand=True, pady=(5, 0))
        
        # 人设结果框
        characters_frame = ttk.LabelFrame(bottom_row, text="👥 人设", padding=5)
        characters_frame.pack(fill=tk.BOTH, expand=True, side=tk.LEFT, padx=(0, 5))
        self._quick_characters_result = tk.Text(characters_frame, wrap=tk.WORD, height=6,
                                                font=(GlassTheme.FONT_FAMILY, GlassTheme.FONT_SIZE_SMALL),
                                                bg=GlassTheme.GLASS_SURFACE, fg=GlassTheme.TEXT_PRIMARY)
        ...
        # 关键情节结果框
        plot_frame = ttk.LabelFrame(bottom_row, text="🎬 关键情节", padding=5)
        plot_frame.pack(fill=tk.BOTH, expand=True, side=tk.RIGHT, padx=(5, 0))
        self._quick_plot_result = tk.Text(plot_frame, wrap=tk.WORD, height=6,
                                          font=(GlassTheme.FONT_FAMILY, GlassTheme.FONT_SIZE_SMALL),
                                          bg=GlassTheme.GLASS_SURFACE, fg=GlassTheme.TEXT_PRIMARY)
        ...
        btn_frame = ttk.Frame(frame, style="TFrame")
        btn_frame.pack(fill=tk.X, padx=5, pady=5)
        ttk.Button(btn_frame, text="一键生成全部", command=self._on_quick_generate_all).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="仅生成世界观", command=lambda: self._on_quick_generate_single("worldview")).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="仅生成大纲", command=lambda: self._on_quick_generate_single("outline")).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="仅生成人设", command=lambda: self._on_quick_generate_single("characters")).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="仅生成情节", command=lambda: self._on_quick_generate_single("plot")).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="导入当前项目", command=self._on_quick_import).pack(side=tk.RIGHT, padx=5)
...
""")
    ...
    def _create_progress_page(self) -> tk.Frame:
        ...
        # 进度统计
        progress_frame = ttk.LabelFrame(frame, text="当前项目状态", padding=20)
    ...
    def _create_project_page(self) -> tk.Frame:
        ...
        # 当前项目信息
        info_frame = ttk.LabelFrame(frame, text="当前项目", padding=20)
    ...
    def _create_plugins_page(self) -> tk.Frame:
        ...
        # 插件列表
        list_frame = ttk.LabelFrame(frame, text="已安装插件", padding=15)
    ...
    def _create_settings_page(self) -> tk.Frame:
        ...
        ttk.Radiobutton(mode_frame, text="本地大模型", variable=self._service_mode_var, value="local").pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(mode_frame, text="线上API", variable=self._service_mode_var, value="online").pack(side=tk.LEFT)
        ...
        key_entry = ttk.Entry(key_frame, textvariable=self._api_key_var, width=40, show="*")
        ...
        ttk.Radiobutton(theme_frame, text="深色", variable=self._theme_var, value="dark").pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(theme_frame, text="浅色", variable=self._theme_var, value="light").pack(side=tk.LEFT)
        ...
        ttk.Spinbox(font_frame, from_=10, to=20, textvariable=self._font_size_var, width=8).pack(side=tk.LEFT, padx=10)
        
        # 窗口大小
        window_frame = ttk.Frame(ui_frame, style="TFrame")
        window_frame.pack(fill=tk.X, pady=5)
        ttk.Label(window_frame, text="默认窗口大小：").pack(side=tk.LEFT)
        self._window_size_var = tk.StringVar(value="1280x720")
        ttk.Combobox(window_frame, textvariable=self._window_size_var, 
                    values=["1280x720", "1366x768", "1920x1080"], width=15).pack(side=tk.LEFT, padx=10)
    ...
    def _on_save_settings(self) -> None:
        ...
        try:
            ...
            self._set_status("设置已保存到config.yaml")
