...
from core.plugin_interface import (
    ...
    ValidatorPlugin,
    PluginContext,
    V5_PROTECTED_MODULES as INTERFACE_PROTECTED_MODULES,
)
...
class TestPluginLifecycle:
    ...
    def test_05_plugin_unregistration(self, plugin_registry):
        """测试插件卸载（UNLOADING）"""
        metadata = PluginMetadata(
            id="test-plugin-005", name="测试插件5", version="1.0.0", plugin_type="TOOL"
        )
    ...
    def test_06_full_lifecycle_flow(self, plugin_registry):
        ...
        assert (
            plugin_registry.get_state("test-plugin-lifecycle")
...
class TestEventBusCommunication:
    ...
    def test_01_publish_subscribe_flow(self, event_bus):
        ...
        handler_id = event_bus.subscribe(
            "test.event", handler, handler_id="test-handler-001"
        )
    ...
    def test_03_priority_order(self, event_bus):
        ...
        event_bus.subscribe(
            "test.priority", handler_normal, priority=EventPriority.NORMAL
        )
...
class TestPluginCollaboration:
    ...
    def test_02_generator_plugin_interface(self, plugin_context):
        ...
        request = GenerationRequest(
            request_id="req-001", title="测试章节", outline="测试大纲", word_count=2000
        )
...
class TestFullBusinessFlow:
    ...
    def test_complete_generation_workflow(
        ...
        # 4. 触发生成流程
        event_bus.publish(
            "generation.started", {"chapter_title": "第一章", "word_count": 2000}
        )
        ...
        event_bus.publish(
            ...
            {"content": "生成内容【本章完】", "word_count": 2000, "score": 0.85},
        )
