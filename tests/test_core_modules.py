...
from core.event_bus import (
    EventBus, EventPriority, DeadLetterQueue,
    get_event_bus
)
from core.plugin_registry import (
    PluginRegistry, PluginState, PluginType,
    get_plugin_registry
)
from core.service_locator import (
    ServiceLocator, ServiceScope, ServiceDescriptor,
    CircularDependencyError, ServiceNotFoundError,
    get_service_locator
)
from core.models import (
    Event, HandlerInfo, PluginMetadata, PluginInfo,
    ValidationScores, GenerationRequest, GenerationResult
)
...
class TestDeadLetterQueue:
    ...
    def test_max_size_limit(self):
        ...
        assert len(items) == 2
        # 应该保留最新的两条
        assert "test_3" in items[0]["event"]["type"] or "test_4" in items[0]["event"]["type"]
